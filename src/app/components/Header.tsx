...
export function Header({ onSync, isSyncing, canSync, onOpenSettings, siteName, siteDescription, logoUrl }: HeaderProps) {
  return (
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="bg-gradient-to-br from-blue-500 to-purple-600 p-2.5 rounded-xl">
              {logoUrl ? (
                <img src={logoUrl} alt={siteName} className="size-6 rounded object-cover" />
              ) : (
                <FileText className="size-6 text-white" />
              )}
            </div>
            <div>
              <h1 className="font-semibold text-xl text-gray-900">{siteName}</h1>
